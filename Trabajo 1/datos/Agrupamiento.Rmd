...
1. accidntabilidad alta critica
1. accidentabilidad alta moderada
2. accidentabiliad moderada
3 acciderntabilidad baja
...
datos_agrupados <- datos_agrupamiento %>% mutate(BARRIO = x_agrupamiento$BARRIO, grupo) %>% 
  mutate(categoria = ifelse(grupo==1, "Accidentabilidad Alta Critica", 
...
left_join(datos_agrupados,barrios_mapa, by = c("BARRIO" = "BARRIO") ) %>% 
  mutate(color = ifelse(grupo==1, "red", 
                            ifelse(grupo == 2,"orange",
                                   ifelse(grupo == 3, "yellow", 
                                            "green"))))-> caracteristicas
```
```{r}
write.csv(caracteristicas, "../datos/caracteristicas_barrios.csv", row.names = FALSE)
```


```{r}
barrios_med %>% inner_join(datos_agrupados,by = c("FECHA_ACCIDENTE" = "BARRIO") )
```
...
m <- addTiles(m)
m=addPolygons(m,popup=caracteristicas$BARRIO,color=caracteristicas$color)
```
