---
title: "Predicción de Accidentabilidad"
date: "26/11/2021"
author: 
...
require(randomForest);library(ridge);library(ranger);library(psych);
library(glmnet);library(rpart);require(randomForest)
...
El preprocesamiento, limpieza y depuración de la base de y todo el codigo para el desarrollo de este proyecto se encuentra en el siguiente [link](https://github.com/duvan314/TAE/tree/main/Trabajo%201).
...
apply(is.na(datos), 2, sum) %>% data.frame() -> data_na
colnames(data_na) <- c("N° Registros Faltantes")

data_na %>% 
  kable(digits = 0, caption = "<center><strong>Tabla 1. Registros faltantes por variable</strong></center>") %>%  
...
El 10.43% es el porcentaje de la información de los datos originales que no se tuvo en cuenta para la realización del trabajo. El procedimiento y el codigo usado en este item se encuentra el el siguiente [script.](https://github.com/duvan314/TAE/blob/main/Trabajo%201/scripts/Preprocesamiento.R)
Finalmente la base de datos limpia contiene  242533 registros y 16 variables, sumadas a las que se presentaron anteriormente, se inclyen las siguientes. 
...
datos <- datos[,-1]
head(datos %>% select(-FECHA_ACCIDENTES)) %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 2. Datos</strong></center>") %>%  
  kable_styling(font_size = 15)
```
...
datos %>%
  ...
  ggtitle("Plot 1a. Accidentes viales por clase de accidente") + theme_light()  +
...
datos %>%
  ...
  theme(legend.title=element_blank()) +
  ggtitle("Plot 1b. Accidentes viales por gravedad de accidente") + theme_light()  +
...
ggplot() +
  ...
  theme(legend.title=element_blank())+
  ggtitle("Plot 2a. Accidentes viales por día de la semana ") + theme_light()+ theme(legend.position = "none") -> p1
...
ggplot() +
  ...
  theme(legend.title=element_blank()) +
  ggtitle("Plot 2b. Accidentes viales por Mes")+ theme_light()+ theme(legend.position = "none") -> p2
...
ggplot() +
  ...
  theme(legend.title=element_blank()) +
  ggtitle("Plot 2c. Accidentes viales por Año") + theme_light() + theme(legend.position = "none")-> p3
...
ggplot(data = datos) +
  ...
  ggtitle("Plot 2c. Densidad Hora de Accidente") + theme_light() -> p4
...
ECM_Caida_diarios %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 3. ECM de entreno y Prueba</strong></center>") %>%  
  kable_styling(font_size = 15)
...
ECM_Atropello_diarios %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 4. ECM de Entreno y Prueba</strong></center>") %>%  
  kable_styling(font_size = 15)
...
plot(mod_ridge_diarios, xvar = "lambda", label = TRUE, xlim = c(-5.5, 10), lwd = 2, main = "Plot 3. Significacias de parametros modelo RIDGE")   
...
ECM_ridge_2020 <- mean((ridge_pred_2020 - y[x[, "AÑO"]==2020])^2)
ridge <- c(ECM_ridge_train, ECM_ridge_test, ECM_ridge_2020)
data.frame(as.matrix(coef_ridge)) %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 5. Coeficientes del modelo RIDGE</strong></center>") %>%  
  kable_styling(font_size = 15)
```
...
ECM_Choque_diarios %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 6. ECM de Entreno y Prueba</strong></center>") %>%  
  kable_styling(font_size = 15)
...
ECM_Otro_diarios %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 8. ECM de Entreno y Prueba</strong></center>") %>%  
  kable_styling(font_size = 15)
...
head(datos_agrupamiento) %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 9. Datos para Agrupamiento</strong></center>") %>%  
  kable_styling(font_size = 15)
...
pairs.panels(datos_agrupamiento[,-1], 
             ...
             , main = "Plot 4. Correlación entre variables")
```


A partir del `Grafico 4`se observa que las variables de accidebtabilidad asociada a los barrios tiene una correlación positiva y alta. Ademaás todas las distribuciones son sesgadas a la derecha indicando que la mayoría de barrios tienen baja frecuecnia de casos. 
...
data.frame(x_agrupamiento,datos_ilustracion_cl[9:17]) %>% head() %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 10. Barrios y grupos </strong></center>") %>%  
  kable_styling(font_size = 15)
```
...
g1 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K2))) + theme_bw() + labs(title = "plot 5a. K=2",
                                      x = "Choque",
                                      y = "Con muertos",colour="Grupo")
g2 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K3))) + theme_bw() + labs(title = "plot 5b.K=3",
                                      x = "Choque",
                                      y = "Con muertos",colour="Grupo")
g3 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K4))) + theme_bw() + labs(title = "plot 5c.K=4",
                                      x = "Choque",
                                      y = "Con muertos",colour="Grupo")
g4 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K5))) + theme_bw() + labs(title = "plot 5d.K=5",
                                      x = "Choque",
                                      y = "Con muertos",colour="Grupo")
...
g1 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K2))) + theme_bw() + labs(title = "plot 6a. K=2",
                                      x = "Proporción accidentes Lunes-Viernes",
                                      y = "Con heridos",colour="Grupo")
g2 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K3))) + theme_bw() + labs(title = "plot 6b. K=3",
                                      x = "Proporción accidentes Lunes-Viernes",
                                      y = "Con heridos",colour="Grupo")
g3 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K4))) + theme_bw() + labs(title = "plot 6c. K=4",
                                      x = "Proporción accidentes Lunes-Viernes",
                                      y = "Con heridos",colour="Grupo")
g4 <- grph_iulstr_cl_k + geom_point(aes(colour = factor(K5))) + theme_bw() + labs(title = "plot 6d. K=5",
                                      x = "Proporción accidentes Lunes-Viernes",
                                      y = "Con heridos",colour="Grupo")
gridExtra::grid.arrange(g1,g2,g3,g4)
```
...
grph_metrica_cl + geom_point(aes(x=num_centros,y=metrica_cl)) +  geom_segment() + theme_bw() + labs(title = "Plot 8. Desempeño del agrupamiento \n en función de K",
...
b1<- grph_bxp + geom_boxplot(aes(group=factor(K4)),orientation="x") + theme_bw() + 
  labs(title = "Plot 9a. Boxplot proporción accidentes en semana",
                                      x = "Grupos",
...
b2<- grph_bxp + geom_boxplot(aes(group=factor(K4)),orientation="x") + theme_bw() + 
  labs(title = "Plot 9b. Boxplot choques para cada grupo",
                                      x = "Grupos",
...
b3<- grph_bxp + geom_boxplot(aes(group=factor(K4)),orientation="x") + theme_bw() + 
  labs(title = "Plot 9c. Boxplot solo daños para cada grupo",
                                      x = "Grupos",
...
b4<- grph_bxp + geom_boxplot(aes(group=factor(K4)),orientation="x") + theme_bw() + 
  labs(title = "Plot 9d. Boxplot muertos para cada grupo",
                                      x = "Grupos",
...
```{r}
cl_medias %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 11. Media en los grupos </strong></center>") %>%  
  kable_styling(font_size = 15) 


cl_median %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 12. Mediana en los grupos </strong></center>") %>%  
  kable_styling(font_size = 15)



cl_sd %>% 
  kable(digits = 5, caption = "<center><strong>Tabla 13. Desviación estándar en los grupos </strong></center>") %>%  
  kable_styling(font_size = 15)  
```
