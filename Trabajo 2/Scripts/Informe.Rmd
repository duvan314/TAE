...
```{r}
pred <- predict(mod_lm, test) 
fit <- predict(mod_lm, train)

sum((pred-mean(test$Units))^2)/
sum((test$Units-mean(test$Units))^2)

sum((fit-mean(train$Units))^2)/
sum((train$Units-mean(train$Units))^2)


1-(sum((test$Units-pred)^2)/
sum((test$Units-mean(test$Units))^2))

1-(sum((train$Units-fit)^2)/
sum((train$Units-mean(train$Units))^2))
