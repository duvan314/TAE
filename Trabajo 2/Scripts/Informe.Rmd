...
Este módulo se presenta con la intención de poder entender los datos, identificar patrones y plantear hipótesis se presentan. 
...
Durante los días martes a viernes no se presentan disimilitudes significativas en el número de Registros, es evidente que el día sábado el número de Registros disminuyen notablemente. Ademas, la mayoría de los Registros del domingo son nulos. La caída de los Registros del dia lunes en comparación a los otros días de la semana podemos asociarlos a los festivos. Por último, para cada dia de la semana se visualizan observaciones atípicas. 
...
En promedio la frecuencia de los registros de vehículos durante los primerios dias del mes es mas baja que durante los demas días. En contraste, la frecuencia promedio de los registros crece al final del mes. 

El comportamiento para los días durante el año es similar al de los dias del mes, es baja al inicio, relativamente contante durante gran parte del año y alta al final. 

$\hspace{1 cm}$

### **Festivos**

Como se habia previsto anteriormente, el número de registros los dias festivos es bajo o nulo. En las siguientes tablas se muestran algunas estadisticas que prueban la hipotesis.
...
$Min\{R^2_{train}/R^2_{Validation}\}$ Los datos de entrenamiento son los registros del *01/01/2012* a *31/12/2016* y validación los registros del 2017 y de prueba serán los datos del primer semestre del 2018. 
...
- Por lo anterior, el modelo posion es adecuado para la predicción de los registros en el RUNT. 
...
Dado que el poisson fue el mejor modelo, se agregará una nueva variable con la intención disminuir la brecha entre el $R^2$ de entreno y validación. 

$\hspace{1 cm}$

#### **Variable dummy**

Considrando que durante los primeros dias del mes los registros en el RUNT son bajos y al final de mes son altos, se crea una función que capture este comportamiento. En este sentido, para la varaible $**\text{Mday}**$ se le aplicará la siguiente funcion. 
...
plot(ts(fv(1:31)), main = "Comportamiento de variable Dummy", xlab = "Dia del mes",ylab= "Valor Dummy", col = "blue4")
```
...
$$log(\text{Units}_t) = \beta_0+\beta_1\text{Yday}_t+\beta_2\text{Year}_t+\beta_{3,k}\text{Month}_{t,k}+
\beta_{4,k}\text{Wday}_{4,k}+\beta_5\text{Mday}_t+\beta_6\text{Holiday}_t+\beta_7\text{Colombia}_t+\beta_8\text{Dummy}_t$$
...
Al agregar la variable Dummy al modelo se consigue mejorar el $R2$ de entreno y validación.
...
Se ajustará un modelo a los resuduales del modelo poisson. 
